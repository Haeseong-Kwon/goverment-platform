...
import { canManagerSeeReviewItem, getDdayTone, getMonthlyDiagnosticUsage, getSidebarItems, getStartupMilestones } from "./logic";
...
function Sidebar({ role }: { role: StartupRole }) {
  return <aside className="hidden w-[240px] shrink-0 border-r border-[#E2E8F0] bg-white p-5 lg:block"><Link href="/" className="text-xl font-bold text-[#0F172A]">StartupOS</Link>{role === "founder" && <p className="mt-1 text-xs font-semibold text-[#2563EB]">인하대학교 창업지원단 연결됨</p>}<nav className="mt-8 space-y-1">{getSidebarItems(role).map((item, index) => <a key={item} className={cn("flex items-center gap-3 rounded-[10px] px-3 py-2.5 text-sm font-semibold", index === 0 ? "bg-[#EFF6FF] text-[#2563EB]" : "text-[#475569] hover:bg-[#F8FAFC]")}><LayoutDashboard size={17} />{item}</a>)}</nav>{role === "pre_founder" && <Link href="/founder/convert" className="mt-8 block rounded-2xl bg-[#EFF6FF] p-4 text-sm font-bold text-[#2563EB]">합격하셨나요?</Link>}</aside>;
}
...
function FounderCore({ founder = false }: { founder?: boolean }) {
  ...
  return <WorkspaceShell role={founder ? "founder" : "pre_founder"}><HeroHeader role={founder ? "founder" : "pre_founder"} /><section className="grid gap-4 md:grid-cols-4"><div className="rounded-2xl bg-[#2563EB] p-5 text-white md:col-span-2"><p className="text-sm font-semibold opacity-90">D-day 히어로</p><h2 className="mt-2 text-2xl font-bold">예창패 서류 마감 D-12</h2><p className="mt-2 text-sm opacity-90">자동 TODO 4개가 생성되었습니다.</p></div>{[["남은 TODO", "7"], ["진단 요약", "88점"], ["팀 진행률", "64%"], ["빠른 계산기", "641,400원"]].map(([label, value]) => <div key={label} className="rounded-2xl border border-[#E2E8F0] bg-white p-5"><p className="text-sm text-[#475569]">{label}</p><strong className="mt-2 block text-2xl tabular-nums">{value}</strong></div>)}</section><section className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_.9fr]"><div className="space-y-6"><div className="rounded-2xl border border-[#E2E8F0] bg-white p-5"><div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-bold">팀 TODO</h2><StatusBadge tone="blue">예창패</StatusBadge></div><div className="grid gap-3 md:grid-cols-2">{todos.map((todo) => <TodoCard key={todo.id} {...todo} />)}</div></div><CalendarPreview /></div><div className="space-y-6"><A1Report kind="eligibility" /><CalculatorCard /><VaultCard /><BizPlanCard exhausted={usage.isExhausted} /><IncorporationCard /><ConnectCard /></div></section></WorkspaceShell>;
}
...
export { FounderCore, Landing, ManagerDashboard, ConvertPage };
